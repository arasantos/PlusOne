...
const _people = handleFindingSimilarPeople();
const MAX_MATCHES = _people.length;
...
for (let i = MAX_MATCHES - 1; i >= 0; i--) {
    ...
    fillPersonInformation(_people[i], cardContent)
...
function handleFindingSimilarPeople() {
    ...
    people.forEach(person => {
        ...
        // first round of sorting, just puts the people that have common interests with user at the front
        if (addPersonToFront) {
            similarPeople.unshift(modifiedPerson);
        } else {
            similarPeople.push(modifiedPerson);
        }
    });

    // second round of sorting, makes the person with most similarities as user, be shown first
...
function goToPersonPage() {
    setTimeout(() => {
        window.location.href = "/userBio.html";
    }, 800);
}
